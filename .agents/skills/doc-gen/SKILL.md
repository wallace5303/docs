...
This skill turns code in the **ee-dev** source project into documentation pages in **this** (ee-doc) VitePress site. Read `AGENTS.md` at the repo root first — it explains the doc conventions you MUST follow (numeric-prefix sorting, frontmatter `title`, auto-generated sidebars, bilingual EN/zh layout).
...
**Any change to a doc page must be mirrored in its Chinese counterpart under `docs/zh/`.** This applies to every kind of edit — creating a new page, modifying content, deleting a section, or removing a page. The two trees must stay in sync: same filenames, same structure, same numeric prefixes. Before finishing, locate the matching `docs/zh/<same-relative-path>` file and apply the equivalent change (translated prose, identical code/blocks/tables). If the zh file does not yet exist, create it; if the EN page is deleted, delete the zh page too. Never leave the EN tree and the `docs/zh/` tree out of sync.

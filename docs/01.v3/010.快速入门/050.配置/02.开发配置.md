...
module.exports = {
  ...
  dev: {
    ...
    electron: {
      ...
      cmd: 'electron',
      args: ['.', '--env=local'],
      // ee-bin@v1.4.0 如果前端启动时间过长，可先加载一个loading页
   ...
   * 举例2：ee-bin build --cmds=go_build_w (构建 go windows平台程序)
   * 举例3：ee-bin build --cmds=go_build_m (构建 go macos平台程序)
   * 举例4：ee-bin build --cmds=go_build_l (构建 go linux平台程序)
   */
  build: {
    frontend: {
      directory: './frontend',
